...
class Cubix:
    ...
    def move_left(self):
        if self.x > 0:
            ...
            print("Warning: cannot move left — already at left edge.")

    def move_right(self):
        if self.x < self.n - 1:
            self.x += 1
        else:
            print("Warning: cannot move right — already at right edge.")

    def move_up(self):
        if self.y > 0:
            self.y -= 1
        else:
            print("Warning: cannot move up — already at top edge.")

    def move_down(self):
        if self.y < self.n - 1:
            self.y += 1
        else:
            print("Warning: cannot move down — already at bottom edge.")
...
def tokenize(code_str):
    ...
    while i < len(code_str):
        if code_str[i] == " " or code_str[i] == "\n":
            i += 1
            continue
        elif code_str[i] == ")":
            print("Unexpected ')' outside of loop.")
            sys.exit(1)
        elif code_str[i] == "[":
            i += 1  # skip '['
            loop_number = ""
            while i < len(code_str) and code_str[i] != "]":
                ...
                i += 1
            i += 1  # skip ']'
            if loop_number.strip() == "":
                print("Error: empty loop count inside [].")
                sys.exit(1)
            try:
                _ = int(loop_number)
            except ValueError:
                print(f"Error: invalid loop count '{loop_number}'")
                sys.exit(1)
    ...
    while pc < code_len:
        ...
        if cmd in ["X'", "Y'", "Z'"]:
            ...
            if 'count' not in cmd or 'body' not in cmd:
                print(f"Malformed loop command: {cmd}")
                sys.exit(1)
